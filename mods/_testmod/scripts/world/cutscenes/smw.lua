return function(cutscene)
    ...
    cutscene:text("* Guess we gonna replace you with Noelle. Whoops", "shock_nervous")

    Game:removePartyMember("kris")
    local noelle = Game:addPartyMember("noelle")
    --print(:convertToPlayer(Game.party[2]))
    --local noelle = Game.getPartyMember("noelle")
    --local noelle = new Character("noelle")
    noelle = Game.world.spawnPlayer(susie.x, susie.y, "noelle")
